...
def operating_point(
    unit_current_design: DipoleDesign, target_bore_field_t: float
) -> OperatingPoint:
    ...
    block_currents = [
        block.current_a for layer in unit_current_design.layers for block in layer.blocks
    ]
...
def operating_point_at_current(
    unit_current_design: DipoleDesign,
    operating_current_a: float,
    *,
    target_bore_field_t: float | None = None,
) -> OperatingPoint:
    """Set an exact current magnitude, choosing polarity toward a field target."""

    if not math.isfinite(operating_current_a) or operating_current_a <= 0.0:
        raise ValueError("operating_current_a must be finite and positive")
    sources = tuple(
        source
        for turn in unit_current_design.all_turns()
        for source in place_line_current_sources(turn)
    ...
    if not block_currents:
        raise ValueError("design must contain at least one block")
    reference_current = block_currents[0]
    if reference_current == 0.0 or not math.isfinite(reference_current):
        raise ValueError("design reference current must be finite and nonzero")
    if any(
        not math.isclose(current, reference_current, rel_tol=1.0e-10, abs_tol=1.0e-10)
        for current in block_currents[1:]
    ):
        raise ValueError("all blocks must carry the same series current")
    scale_magnitude = operating_current_a / abs(reference_current)
    if target_bore_field_t is None:
        scale_factor = scale_magnitude
    else:
        if not math.isfinite(target_bore_field_t) or target_bore_field_t == 0.0:
            raise ValueError("target_bore_field_t must be finite and nonzero")
        scale_factor = math.copysign(scale_magnitude, target_bore_field_t / by_t)
    signed_operating_current_a = reference_current * scale_factor
    return OperatingPoint(
        unit_bore_field_t=by_t,
        scale_factor=scale_factor,
        operating_current_a=signed_operating_current_a,
        design=scale_design_currents(unit_current_design, scale_factor),
    )
